		...
		try
	    {
			...
			if(myFile.isMissing()){
				...
				if (file_size < file_size_max) {
					...
					//String filedate = String.valueOf(calendar.getTimeInMillis());
					//Date currentTime = new Date();//得到当前系统时间
					
					//String str_date = currentTime.toString(); //将Date型日期时间转换成字符串形式
					//Calendar calendar = Calendar.getInstance();
					//String filename = String.valueOf(calendar.getTimeInMillis());
					String filename = upload.getRequest().getParameter("username")+"_"+upload.getRequest().getParameter("sourceId")+"_"+year+"_"+month+"_"+day ;//+"_"+str_date;//根据用户名和作业id号命名
